...
def main(path, lr, bs, device):
    ...
    for i, t_value in enumerate(ts):
        ...
        for j, batch in enumerate(train_loader):
            ...
            train_pred, features, _ = model(train_data, True, t)
            ...
            avg_cost[i, :6] += cost[:6] / train_batch
        ...
        with torch.no_grad():  # operations inside don't track history
            ...
            for k in range(test_batch):
                ...
                avg_cost[i, 6:] += cost[6:] / test_batch

            # compute mIoU and acc
            avg_cost[i, 7:9] = conf_mat.get_metrics()

            # Test Delta_m
            test_delta_m = delta_fn(
                avg_cost[i, [7, 8, 10, 11]]
            )
            ...
            print(
                ...
                f"TEST: {avg_cost[i, 7]:.4f} {avg_cost[i, 8]:.4f} {avg_cost[i, 9]:.4f} | "
                f"{avg_cost[i, 10]:.4f} {avg_cost[i, 11]:.4f}"
                f"| {test_delta_m:.3f}"
            )
