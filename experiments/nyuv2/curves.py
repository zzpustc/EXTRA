...
# Define the NURBS class  
class NURBS(Module):
    def __init__(self, num_ctrl_points, knot_vector=None):
        super(NURBS, self).__init__()
        self.num_ctrl_points = num_ctrl_points
        self.degree = 3
        self.weights = torch.nn.Parameter(torch.rand(num_ctrl_points))

        if knot_vector is None:
            self.knot_vector = self.create_uniform_knot_vector()
        else:
            self.knot_vector = torch.tensor(knot_vector, dtype=torch.float32)

    def create_uniform_knot_vector(self):
        num_knots = self.num_ctrl_points + self.degree + 1
        knots = torch.cat((
            torch.zeros(self.degree),
            torch.linspace(0, 1, num_knots - 2 * (self.degree)),
    ...
    def basis_function(self, i, k, t):
        if k == 0:
            if i < len(self.knot_vector) - 1:
                return ((self.knot_vector[i] <= t) & (t < self.knot_vector[i + 1])).float()
            else:
                return torch.zeros_like(t)
        else:
            left = torch.zeros_like(t)
            if i < len(self.knot_vector) - k and self.knot_vector[i + k] > self.knot_vector[i]:
                left = ((t - self.knot_vector[i]) / (self.knot_vector[i + k] - self.knot_vector[i])) * self.basis_function(i, k - 1, t)

            right = torch.zeros_like(t)
            if i + 1 < len(self.knot_vector) - k and self.knot_vector[i + k + 1] > self.knot_vector[i + 1]:
                right = ((self.knot_vector[i + k + 1] - t) / (self.knot_vector[i + k + 1] - self.knot_vector[i + 1])) * self.basis_function(i + 1, k - 1, t)

            return left + right

    def forward(self, t):
        numerators = []
        denominator = torch.zeros_like(t).cuda()
        
        for i in range(self.num_ctrl_points):
            Ni = self.basis_function(i, self.degree, t).cuda()
            # print(Ni)
            numerator = Ni * self.weights[i]
            # print(denominator)
            numerators.append(numerator)
            denominator += numerator

        numerators = torch.stack(numerators, dim=0)
        coefficients = numerators / (denominator + 1e-8)  # Normalize to get final coefficients
        return coefficients
...
class Bezier(Module):
    ...
    def forward(self, t):
        self.binom = torch.Tensor(binom(self.num_bends - 1, np.arange(self.num_bends))).cuda()
        scale_factor = 1.e-2
        rand_control = torch.rand(1)
        rand_perturb = torch.rand(self.num_bends - 2)
        add_on = torch.zeros(self.num_bends)
        add_on[1:-1] = rand_perturb
        eplise = add_on * scale_factor
        if rand_control > 0.5:
            self.binom += eplise.cuda()   #random perturbation
        else:
            self.binom -= eplise.cuda()   #random perturbation
        return self.binom * \
...
class CurveNet(Module):
    def __init__(self, curve, architecture, num_bends, fix_start=True, fix_end=True):
        ...
        self.net = self.architecture(self.fix_points)
        self.curve_modules = []
    ...
    def forward(self, input, return_representation=False, t=None):
        if t is None:
            t = input.data.new(1).uniform_()
        coeffs_t = self.coeff_layer(t)

        output, rep = self.net(input, coeffs_t, return_representation)
        self._compute_l2()
