...
from curves import CurveNet, Bezier, NURBS, l2_regularizer
        ...
        for j, batch in enumerate(train_loader):
            ...
            losses_asst = torch.stack(
                (
                    ...
                    calc_loss(train_pred_asst[2], train_normal, "normal"),
                )
            )

            pareto_weight = 2 * (torch.exp(-torch.abs(t1-0.5)) + torch.exp(-torch.abs(t2 - 0.5))) / 2.0
            loss_sum = (losses[0] + losses_asst[0]) / 2 + 10 * (losses[2] + losses_asst[2]) / 2 + pareto_weight * (losses[1] + losses_asst[1]) / 2 # loss 2 should be weighted.
            
            loss_order = (torch.exp((losses[0] - losses_asst[0]) * (t2 - t1)) + torch.exp((losses[2] - losses_asst[2]) * (t1 - t2))) / 2.0
            loss = loss_sum + loss_order
...
if __name__ == "__main__":
    ...
    parser.add_argument('--num_bends', type=int, default=5, metavar='N',
